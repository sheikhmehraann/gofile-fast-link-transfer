"""Orchestrated link-to-GoFile pipeline connecting resolvers, downloader, and uploader."""
...
class TransferPipeline:
    """End-to-end pipeline to resolve, download, upload, and format output."""

    def __init__(self, connections: int = 8, gofile_token: Optional[str] = None, keep_files: bool = False):
    ...
    def process_url(self, url: str, output_dir: Optional[str] = None, folder_id: Optional[str] = None) -> TransferSummary:
        """Process a single URL: Resolve -> Download -> Upload to GoFile."""
        ...
        console.print(f"[bold white]File Name:[/bold white] {resolved.filename} | [bold white]Size:[/bold white] {resolved.file_size or 'Unknown'} bytes")
        ...
        try:
            ...
            # 2. Upload Phase
            start_ul = time.time()
            gofile_res: GoFileResult = self.uploader.upload(local_path, folder_id=folder_id)
